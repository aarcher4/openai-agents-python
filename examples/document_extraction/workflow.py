...
class WorkflowInput(BaseModel):
    ...
    input_as_file_url: str | None = None
    input_as_file_path: str | None = None
    input_as_file_data: str | None = None
...
def _build_conversation_history(workflow_input: WorkflowInput) -> list[TResponseInputItem]:
    ...
    if workflow_input.input_as_file_url:
        content.append({"type": "input_file", "file_url": workflow_input.input_as_file_url})

    if workflow_input.input_as_file_path:
        file_data = _encode_image_path(workflow_input.input_as_file_path)
        content.append(
            {
                "type": "input_file",
                "file_data": file_data,
                "filename": Path(workflow_input.input_as_file_path).name,
            }
        )

    if workflow_input.input_as_file_data:
        content.append(
            {
                "type": "input_file",
                "file_data": workflow_input.input_as_file_data,
    ...
    if not content:
        raise ValueError(
            "At least one of input_as_text, input_as_image_url, input_as_image_path, input_as_file_url, input_as_file_path, or input_as_file_data must be provided."
        )
